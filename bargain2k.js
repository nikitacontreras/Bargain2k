...
(() => {
    ...
    class processor {
        ...
        TAB(question) {
            options = options.length <= 0 ? question.responses.flatMap(response =>
    ...
    function collectResponsesByType(labs) {
        ...
        for (const { exercises } of labs) {
            for (const { _data, questions, type } of exercises) {
                const isQuiz = _data.some(item =>
                    item.type === "group" &&
                    Array.isArray(item.contents) &&
                    item.contents.some(c => c.content === "Quiz")
                );
                for (const q of questions) {
                    for (const r of q.responses.filter(r => r.correct === true || r.correct === 1)) {
    ...
    function CustomXHR() {
        ...
        xhr.onreadystatechange = function () {
            if (this.readyState === 4 && this.isLessonRequest && this.status === 200) {
                try {
                    lessonData = JSON.parse(this.responseText);

                    if (lessonData && Array.isArray(lessonData.labs)) {
                        const { quizResponses, practiceResponses } = collectResponsesByType(lessonData.labs);


                        crearVentana('ventana-quiz', 'Quiz Answers', quizResponses);
                        crearVentana('ventana-practice', 'Practice Answers', practiceResponses, 0 , 50);
                    } else {
